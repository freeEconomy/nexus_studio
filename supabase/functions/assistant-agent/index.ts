  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          service: { type: ['string', 'null'], description: '서비스 구분 (MC 또는 MS). 전체 조회 시 생략 가능' },
          jql: { type: ['string', 'null'], description: '직접 입력할 JQL 쿼리' },
          issueKey: { type: ['string', 'null'], description: '단일 Jira 이슈 키 (예: PROJ-123)' },
        },
      },
    },
  },
...
async function executeTool(name: string, args: any, supabaseUrl: string, serviceKey: string) {
  ...
  if (name === 'query_jira') {
    ...
    // 결과가 너무 길어지지 않도록 최대 15개로 제한
    return { issues: combinedIssues.slice(0, 15) }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const FALLBACK_SYSTEM_PROMPT = `당신은 MAMF 회사의 AI 비서 "Nexus Agent"입니다.

[응답 규칙]
- **절대 HTML 태그를 사용하지 마세요.**
- **내부적인 툴 호출 과정(예: "get_tasks를 호출했습니다" 등)을 절대 설명하지 마세요.**
- **카테고리 분류(기획/개발/디자인 등)를 하지 마세요.**
- 모든 응답은 마크다운 형식을 사용하고, 한국어로 친절하게 답변하세요.

[Jira 관련 정보]
- MC 에픽: MAMF-1645
- MS 에픽: MAMF-1633
- 상태가 '등록(To Do)' 또는 '진행중(In Progress)'인 이슈를 기본으로 조회합니다.

[중요 지침: 지라 이슈 비교 및 등록]
1. 사용자가 "MC 지라 이슈 알려줘" 또는 "등록 안 된 지라 이슈 확인해줘"라고 요청하면:
   - 먼저 get_tasks를 호출하여 현재 등록된 업무 목록을 가져옵니다.
   - 그 다음 query_jira를 호출하여 해당 서비스(예: MC)의 지라 이슈들을 가져옵니다.
   - 지라 이슈의 'key'와 DB 업무의 'issue' 필드를 비교하여, **DB에 아직 등록되지 않은 지라 이슈만 골라냅니다.**
2. 결과 출력 형식:
   - 서두: "현재 지라에는 있지만 업무 리스트에는 등록되지 않은 이슈들입니다."
   - 목록: [이슈키] 제목 (상태) 형식으로 나열하세요.
   - 마무리: "이 중 업무 리스트에 등록하고 싶은 이슈가 있으신가요? 이슈 키를 말씀해주시면 바로 등록해 드릴게요."
3. 사용자가 특정 이슈를 등록해달라고 하면:
   - add_task 툴을 사용합니다.
   - title: 지라의 summary
   - issue: 지라의 key (예: MAMF-1905)
   - description: 지라 상세 페이지 링크와 함께 간단한 설명을 포함하세요.
   - service: 해당 서비스 (MC 또는 MS)
   - status: 'received'로 설정`
